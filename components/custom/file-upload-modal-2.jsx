...
const FileUploadModal = ({
  ...
  return (
    ...
    >
      <div className="flex w-full flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 p-8 text-slate-700 dark:border-slate-700 dark:text-slate-300">
        ...
        <div className="group">
          <label
            ...
            className="cursor-pointer font-medium text-orange-700 group-focus-within:underline dark:text-orange-600"
          >
            <input
      ...
      {filename && (
        <p className="mt-2 text-md text-center text-orange-700">
      ...
      {uploading && (
        <IconProgress
          size={15}
          className="mr-3 mt-3 h-7 w-5 animate-spin text-orange-700 justify-center"
        />
      )}

      {uploadSuccess && (
        <p className="mt-2 text-orange-600">Upload successful!</p>
      )}
      {/* <div className="justify-center items-center flex flex-col -mb-3">
        <Input placeholder="Enter the title of the document" className="m-3 w-4/5" />
      </div> */}
    </Modal>
  );
};
